...
const Pagination = function (props: PaginationProps) {
  const { total, pageIndex, pageSize, onPaginationChange } = props;
  ...
  const pageBtns = useMemo(() => {
    ...
    while (i + 1 <= maxPageCount && i + 1 <= pageIndex + 2) {
      i += 1;
      bottons.push(<PageBtn key={i} index={i} href={getHref(i)} />);
    }
    if (i + 1 === maxPageCount) {
      bottons.push(
        <PageBtn
    ...
    while (i - 1 >= 1 && i - 1 >= pageIndex - 2) {
      i -= 1;
      bottons.unshift(<PageBtn key={i} index={i} href={getHref(i)} />);
    }
    if (i - 1 === 1) {
      bottons.unshift(<PageBtn key={1} index={1} href={getHref(1)} />);
    } else if (i - 1 > 1) {
      bottons.unshift(
        <PageBtn key={1} index={1} href={getHref(1)} />,
